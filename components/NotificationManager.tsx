...
const NotificationItem: React.FC<{ notif: DeadlineNotification, onDismiss: (id: string) => void }> = ({ notif, onDismiss }) => {
  ...
  return (
    <div className={`pointer-events-auto group relative flex items-start gap-4 p-5 rounded-[1.8rem] border-2 bg-white/80 backdrop-blur-2xl shadow-2xl animate-slide-right transition-all hover:scale-[1.02] ${
      ...
      <button 
        ...
        className="absolute top-4 right-4 p-1 text-slate-300 hover:text-slate-600 transition-colors"
      >
        <X size={16} />
      </button>
